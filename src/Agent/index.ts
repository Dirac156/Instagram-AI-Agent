...
//     const parser = new JsonOutputKeyParser({ keyName: "comment" }); // adjust if your key is different

//     const fullPrompt = ChatPromptTemplate.fromMessages([
//       ["system", "You are a friendly and professional Instagram commenter."],
//       ["human", prompt],
//     ]);

//     const chain = fullPrompt.pipe(model).pipe(parser);

//     const result = await chain.invoke({});
//     return [{ comment: result }]; // wrap to match your previous structure
//   } catch (error) {
//     logger.error("Error generating comment with LLaMA:", error);
